...
#import "PDJStyleViewController.h"
#import "TBXML.h"
...
@interface PDJStyleViewController ()
...
@implementation PDJStyleViewController
@synthesize adBanner;
...
- (void)viewDidLoad
{
   ...
   // self.tableView.backgroundView = [[UIImageView alloc] initWithImage:    /// set background tableview
    ...
    NSString *myPodcast = categoryPlaylistPromo;
...
- (void)APIDownload:(APIDownload*)request {
    ...
    [self.tableView reloadData];
    [self writeToPlist:@"mixes-1.plist" withData:self.myPodcast];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [self performSegueWithIdentifier:@"List2" sender:indexPath];
    NSLog(@"tap");
...
{
    if([[segue identifier] isEqualToString:@"List2"])
    {
        PDJPodcastViewController *playerz = [self.storyboard instantiateViewControllerWithIdentifier:@"PDJPodcastViewController"];
